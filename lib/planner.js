...
// Manual "mark as posted" (for platforms without a wired publisher yet).
...
// Record the outcome of a real publish attempt (from the publishing engine).
// result: { platform, ok, url?, id?, error? }
function recordPublish(planner, id, result) {
  const post = findPost(planner, id);
  post.results = post.results || {};
  post.results[result.platform] = result.ok
    ? { ok: true, url: result.url, id: result.id, at: nowIso() }
    : { ok: false, error: result.error, at: nowIso() };
  if (result.ok) {
    post.status = 'posted';
    post.postedAt = nowIso();
  } else {
